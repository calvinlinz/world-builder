...
import styles from "../components/configuration/configuration.module.css";
import MonstersOverlay from "../components/monstersOverlay/MonstersOverlay";
...
const MonsterGrid = memo(function MonstersGrid({worldData, scaleFactor}){
    ...
    const handleClickOutside = (event) => {
      ...
      if (monsterBox && !monsterBox.contains(event.target)) {
        console.log("OUT");
      }
    };
...
const cha2Map = {
  ...
  if (selectedMonsterOption === "none") {
    ...
    const handleSelectChange = (value) => {
      setSelectedMonsterOption(value);
    };
    ...
    const allMonsterImages = monsterCords.map((image, index) => {
        ...
        return (
          ...
          <img
            ...
            style={imageStyle}
            onClick={()=>handleSelectChange(image)}
          />
          {contentToRender}
          </>
